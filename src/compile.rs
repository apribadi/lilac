...
fn pop_points(n: u32, e: &mut Env) -> impl Iterator<Item = Point> {
  return e.points.drain(e.points.len() - n as usize ..);
}
...
impl What {
  ...
  fn into_points(self, e: &mut Env, o: &mut Out) -> u32 {
    match self {
      What::NumPoints(n) => {
        return n;
      }
      What::NumValues(n) => {
        ...
        let i = o.emit_point(Some(n as u32));
        put_point(i, e);
        return 1;
      }
    }
